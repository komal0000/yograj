...
<head>
    @php
        $meta = App\Helper::getMetaSetting();
    @endphp
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <meta property="og:title" content="{{ $meta->meta_title  ?? 'Default Title' }}">
    <meta property="og:description" content="{{ $meta->meta_description ?? ' Description' }}">
    <meta property="og:image" content="{{ asset($meta->meta_image ?? 'image.png') }}">
    <meta property="og:url" content="{{ url()->current() }}">
    <title>{{ $meta->meta_title  ?? '  Title' }}</title>
